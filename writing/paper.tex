...
\usepackage{amsmath, mathtools, natbib, graphicx, placeins, tabularx,booktabs, tikz, pdflscape, subcaption}
\usetikzlibrary{calc}
%\usepackage[. . .]{endfloat}
%\usepackage{floatrow}
...
%markedspris reelt var 107.25 DKK per aktie (totalt 31.5 mia.DKK) i 2014, og altså lig med den pris som  Finansministeriet og DONG har udtalt at DONG's egenkapital blev handlet til. Desuden antager vi, at den fremtidige vækst i DONG's værdi trækkes fra den historiske afkastfordeling fra europæiske elektricitetsselskaber. Med disse antagelser bliver det forventede tab på aftalen på ca.\ 2.5 mia.\ DKK. Sandsynligheden for at den indgåede aftale giver et bedre afkast end det offentlige alternativ er cirka 50\%, men qua den asymmetriske afkaststruktur, hvor staten tager en del af downside, men ikke får nogen upside, opvejer det bedre afkast, i scenarier hvor værdien af DONG's egenkapital falder, ikke det afkast staten går glip af i  scenarier, hvor DONG's egenkapital stiger i værdi.
...
\begin{figure}
\caption{Illustration af perioder i statens aftale med de private investorer.}
\label{fig:perioder}
\includegraphics[scale=0.6]{../figs/perioder}
\caption*{\raggedright Kilde: \citet{FM2013a}.}
\end{figure}
...
På baggrund af dette kan statens absolutte afkast på aftalen illustreres som gjort i Figur \ref{fig:privat_off}. Med antagelsen om salgsrettens eksekvering kan statens afkast på aftalen skrives som % hvor $K$ er kapital indskuddet (11 mia. DKK).
%&P= 
%$P=\begin{dcases} 
%-0{,}6K \prod_{i=2014}^{2017} (1+g_i)+0{,}6K(1+r)^n +\Omega&\text{     hvis    } 0{,}6\prod_{i=2014}^{2017}(1+g_k)+0{,}4(1+r)^n>(1+r)^n \\ 
%\Omega  &\text{   ellers } 
%\end {dcases} $.\\
%\text{Her er } $\Omega=(1+r)^n\left( \frac{sg_0 V_0}{V_0+K} \left( V_m+K \right)-sg_0 V_m \right)$.
...
\begin{figure}
\caption{Beregnet optionsvolatilitet for globale energiaktier.}
\centering
   \begin{subfigure}[b]{0.8\textwidth}
   \caption{Forskellige optionsvarigheder.}
   \label{fig:option_vol_curve}
   \includegraphics[width=1\linewidth]{../matlab/figs/implied_vol_curve.png} 
\end{subfigure}
\begin{subfigure}[b]{0.8\textwidth}
   \caption{Optionsvarighed på 730 dage.}
   \label{fig:option_vol_730}
   \includegraphics[width=1\linewidth]{../matlab/figs/implied_vol_730.png}
\end{subfigure}
\caption*{\raggedright Kilde: Shares Global Energy, IXC.}
\end{figure}
...
% Jeg har ændret dette til tabular fordi jeg havde problemer. Du kan ændre tilbage hvis det fungerer for dig.
\begin{center}
	\begin{tabular}{cXcccccr}
	\toprule[1pt] 
...
%I vores benchmark beregning finder vi optionens værdi til 10.83 DKK per aktie, hvorfor de private investorer reelt blot betalte 96.42 DKK  per aktie svarende til en værdi på DONG's egenkapital på 27.2 mia.\ DKK. %I vores benchmark beregning finder vi prisen på optionen på 17.8 DKK hvormed prisen per aktie bliver 89.43 DKK og den totale pris for DONG's egenkapital bliver 26.27 mia. DKK.
...
\label{sec:app_tree}
Lad os først gøre et par generelle antagelser for at gøre problemet løsbart. For det første antager vi, at NEI har en præference for, at børsnotere DONG hvis markedsprisen for DONG aktier er over den garanterede tilbagekøbspris.\footnote{Dette er ikke en triviel antagelse, idet NEI i denne situation også vil kunne vælge at sælge aktierne tilbage til staten til markedsprisen. Vores resultater vil dog være identiske hvis vi antog modsat.} For det andet antager vi, at staten, i tråd med den indgåede politiske aftale om DONG, foretrækker at børsnotere DONG. %TBD: ATP.
...
%De private investorer betalte 107 kroner pr aktie for deres investering i DONG. Denne pris afspejlede imidlertid både prisen for DONG's egenkapital samt en put-option på 60\% af aktierne. Dvs.
...
%Samlet pris for optionen bliver derfor: antal solgte aktier*0.6*29.7=1.8 mia.  Og den initiale værdiansættelse af DONG's egenkapital bliver derfor: (107-0.6*29.7)*antal aktier før kapital udvidelse=26.2 mia. 
